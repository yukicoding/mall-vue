<template>
  <div>
    ...
    <el-table :data="tableData" border stripe :header-cell-class-name="'headerBg'"
      ...
      <el-table-column prop="content" label="商品详情">
        <template slot-scope="scope">
          <el-button @click="view(scope.row.content)" type="primary">详情</el-button>
        </template>
      </el-table-column>
    ...
    <el-dialog title="商品信息" :visible.sync="viewDialogVis" width="60%" >
      <el-card>
        <div>
          <mavon-editor
              class="md"
...
export default {
  ...
  methods: {
    ...
    changeEnable(row) {
      this.request.post("/product/update", row).then(res => {
        if (res.code === '200') {
